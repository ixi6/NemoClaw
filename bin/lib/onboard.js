...
async function setupNim(sandboxName, gpu) {
  ...
  if (options.length > 1) {
    ...
    if (selected.key === "nim") {
      ...
      let downloadModel; // full download ID (e.g., "qwen3-0.6b@q4_k_m")
      if (isNonInteractive()) {
        const rec = getRecommendedSidecarModel(providerKey, gpu ? gpu.perGpuMB : 0);
        downloadModel = requestedModel || (rec ? rec.model : getSidecarModelTiers(providerKey).pop().model);
      } else {
        downloadModel = await promptSidecarModel(gpu, providerKey);
      }
      ...
      if (!sidecar.hasModel("default", downloadModel)) {
        const pullArgs = sidecar.getPullArgs(sidecar.containerName("default"), downloadModel);
