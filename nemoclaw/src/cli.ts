...
export function registerCliCommands(ctx: PluginCliContext, api: OpenClawPluginApi): void {
  ...
  nemoclaw
    ...
    .option("--provider <type>", "Provider type: build, openai, anthropic, gemini, ncp, ollama, custom (nim-local and vllm are experimental)")
    .option("--endpoint <type>", "Deprecated alias for --provider")
    .option("--ncp-partner <name>", "NVIDIA Cloud Partner name (when provider is ncp)")
    .option("--endpoint-url <url>", "Endpoint URL override (for openai, anthropic, gemini, ncp, nim-local, ollama, or custom)")
